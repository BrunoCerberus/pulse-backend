...
- `supabase/migrations/036_search_articles_language_offset.sql` - `search_articles` gains `search_language` (ISO 639-1; NULL = all languages, so existing two-argument callers keep today's behaviour) + `result_offset`, replacing the two-argument signature outright (PostgREST resolves RPCs by argument name — an overload would be an ambiguity waiting to 300): search now honours the app's user-selected language, and iOS "load more" gets real pagination instead of re-fetching page 1. Same hardening as 027: `search_path = ''`, 3s `statement_timeout`, EXECUTE to anon/authenticated/service_role only.
