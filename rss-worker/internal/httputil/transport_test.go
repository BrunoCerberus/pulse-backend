...
func TestIsForbiddenIP(t *testing.T) {
	...
	cases := []struct {
		...
		{"this-network 0.0.0.1 (routes to localhost on Linux)", net.ParseIP("0.0.0.1"), true},
		{"this-network 0.255.255.255", net.ParseIP("0.255.255.255"), true},
		{"this-network v4-mapped", net.ParseIP("::ffff:0.1.2.3"), true},
		...
		{"6to4 relay anycast 192.88.99.x (RFC 7526)", net.ParseIP("192.88.99.1"), true},
		{"Class-E reserved 240.x (RFC 1112)", net.ParseIP("240.0.0.1"), true},
		{"limited broadcast 255.255.255.255", net.ParseIP("255.255.255.255"), true},
